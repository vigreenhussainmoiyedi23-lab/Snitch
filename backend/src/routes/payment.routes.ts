...
import { GetCartWithTotalPrice } from "../services/cart.service.js";
...
paymentRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const cart = await GetCartWithTotalPrice(req.user!._id.toString());

    if (!cart || cart.cartItems.length === 0) {
      throw new AppError("Cart is empty", 400);
    }
    const shipping = cart.totalAmount > 1500 ? 0 : 100;
    const totalAmount = (cart.totalAmount + shipping) * 1.18;
  ...
  try {
    ...
    if (result) {
      ...
      await orderModel.create({
        ...
        productsPurchased: cart.cartItems,
      });
