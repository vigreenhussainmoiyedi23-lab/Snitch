...
const CartSummary: React.FC<CartSummaryProps> = ({ totalAmount, isCheckoutPage = false }) => {
  const shipping = totalAmount > 0 ? 0 : 0; // Placeholder for free shipping
  ...
  return (
    <div className="bg-[var(--color-background-light)] p-5 sm:p-6 rounded-[var(--radius-lg)] shadow-[var(--shadow-soft)] border border-[var(--color-border)] flex flex-col gap-6 w-full animate-in fade-in zoom-in-95 duration-500">
      {!isCheckoutPage && <h3 className="font-semibold text-lg text-[var(--color-text)] tracking-wide">Order Summary</h3>}
      
      <div className="flex flex-col gap-3 text-sm">
        <div className="flex justify-between items-center text-[var(--color-text-muted)]">
          <span>Subtotal</span>
          <span className="font-medium text-[var(--color-text)]">₹{totalAmount.toFixed(2)}</span>
        </div>
        <div className="flex justify-between items-center text-[var(--color-text-muted)]">
          <span>Shipping</span>
          {/* <span className="font-medium text-[var(--color-text)]">{shipping === 0 ? 'Free' : `₹${shipping.toFixed(2)}`}</span> */}
        </div>
        {discount > 0 && (
      ...
      <div className="pt-4 border-t border-[var(--color-border)] flex justify-between items-center text-lg font-semibold text-[var(--color-text)]">
        <span>Total</span>
        <span className="tracking-wide">₹{finalTotal.toFixed(2)}</span>
      </div>
